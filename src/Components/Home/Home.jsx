...
const Home = () => {
  ...
  let { handleMockState, handleMockData, handleIdMock } =
  ...
  return isLoading ? (
    ...
    <>
      ...
      <div className={styles.trending}>
        <span onClick={() => handleSearch('Covid')} style={{ color: 'red' }}>
          COVID-19:{' '}
        </span>
        <span onClick={() => handleSearch('Covid')}>Live Updates</span>
        <span onClick={() => handleSearch('Vaccination in all countries')}>
          Vaccinations by Country
        </span>
        <span style={{ color: 'red' }}>TRENDING: </span>
        {trending?.map((item) => (
          <span
            onClick={() => goToLinkMock(item.id)}
            {...item}
            trend={true}
            key={item.id}
          >
      ...
      <MainContainer>
        <TopNewsContainer>
          <TopNewsLeftPart>
            <img
              src='https://s.yimg.com/uu/api/res/1.2/4148lV0Gc.a_NK8g.N1egA--~B/aD05MDA7dz0xNjAwO2FwcGlkPXl0YWNoeW9u/https://s.yimg.com/os/creatr-uploaded-images/2021-02/38b44370-7840-11eb-9dff-c6bc9a7d303a.cf.jpg'
              alt='logo'
            ></img>
